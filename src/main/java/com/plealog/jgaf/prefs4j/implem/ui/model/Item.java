...
import com.plealog.jgaf.prefs4j.ui.ItemException;
...
public class Item extends AbstractEntry {
  ...
  /**
   * Utility method. Returns the CType integer representation given its string representation.
   *
   * @param sCType the string representation of the container type. Should be one of choice or atom.
  ...
  public static int getCType(String sCType) {
    if (sCType.equals(S_CTYPE_CHOICE)) return CTYPE_CHOICE;
    else if (sCType.equals(S_CTYPE_VALUE)) return CTYPE_VALUE;
    else throw new ItemException("Unknown CType: " + sCType);
  }

  /**
   * Utility method. Returns the string representation of CType given its integer representation.
   *
   * @param cType the integer representation of the container type. Should be one of CTYPE_CHOICE
   *     (1) or CTYPE_VALUE (2).
   * @return one of CTYPE_CHOICE or CTYPE_VALUE
   * @throws ItemException if cType is invalid.
   */
  public static String getSCType(int cType) {
    if (cType == CTYPE_CHOICE) return S_CTYPE_CHOICE;
    else if (cType == CTYPE_VALUE) return S_CTYPE_VALUE;
    else throw new ItemException("Unknown CType: " + cType);
  }

  /**
   * Utility method. Returns the integer representation of DType given its string representation.
   *
   * @param sDType the string representation of the data type. Should be one of integer, real,
   *     string or bool.
  ...
  public static int getDType(String sDType) {
    if (sDType.equals(S_DTYPE_INT)) return DTYPE_INT;
    else if (sDType.equals(S_DTYPE_REAL)) return DTYPE_REAL;
    else if (sDType.equals(S_DTYPE_STRING)) return DTYPE_STRING;
    else if (sDType.equals(S_DTYPE_BOOLEAN)) return DTYPE_BOOLEAN;
    else if (sDType.equals(S_DTYPE_FOLDER)) return DTYPE_FOLDER;
    else if (sDType.equals(S_DTYPE_FILE)) return DTYPE_FILE;
    else if (sDType.equals(S_DTYPE_FILES)) return DTYPE_FILES;
    else throw new ItemException("Unknown DType: " + sDType);
  }

  /**
   * Utility method. Returns the string representation of DType given its integer representation.
   *
   * @param dType the integer representation of the data type. Should be one of DTYPE_INT (1),
   *     DTYPE_REAL (2), DTYPE_STRING (3) or DTYPE_BOOLEAN (4).
  ...
  public static String getSDType(int dType) {
    if (dType == DTYPE_INT) return S_DTYPE_INT;
    else if (dType == DTYPE_REAL) return S_DTYPE_REAL;
    else if (dType == DTYPE_STRING) return S_DTYPE_STRING;
    else if (dType == DTYPE_BOOLEAN) return S_DTYPE_BOOLEAN;
    else if (dType == DTYPE_FOLDER) return S_DTYPE_FOLDER;
    else if (dType == DTYPE_FILE) return S_DTYPE_FILE;
    else if (dType == DTYPE_FILES) return S_DTYPE_FILES;
  ...
  /**
   * Sets the default value associated to thie Item. Returns null if not default value is associated
   * to this Item.
   */
  ...
  /**
   * Returns the Range associated to this Item. Returns null if not Range is associated to this
   * Item.
   */
  ...
  /**
   * Sets the Choice associated to this Item. A Choice is used to set values for container type
   * CTYPE_CHOICE.
   */
  ...
  /**
   * Returns the Choice associated to this Item. Returns null if not Choice is associated to this
   * Item.
   */
   ...
   * @throws ItemException if obj content is invalid regarding the definition of this item
  ...
  public void controlValue(String obj) throws ItemException {
    ...
    if (obj == null)
      throw new ItemException(
          getName() + ": " + EZEnvironment.getMessage("__EZItem.control.error.null"));
