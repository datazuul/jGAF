...
/**
 * This class is responsible for managing the Blast parameters. Those parameters are read by this
 * class from a well-formatted properties file.
 *
...
public class PropertiesModel {
   ...
   */
  public PropertiesModel(ResourceBundle localizedMessages, ResourceBundle model)
  ...
  private FormAction createFormAction(String cEntryKey, String actionKey) {
    ...
    if (val == EZEnvironmentImplem.UNKNOWNSTRING)
      throw new ItemException(
          "Choice entry is wrongly define: action not found for " + cEntryKey + ": " + actionKey);
    actionItems = EZEnvironmentImplem.tokenize(val, ":\t\n\r\f");
    if (actionItems.length != 3)
      throw new ItemException(
          "Choice entry is wrongly define: action does not contain the 3 items for "
              + cEntryKey
              + ": "
              + actionKey);
    ...
    if (actionName.equals(SET_VALUE_ACTION_KEY)) {
      ...
      if (values.length == 0)
        throw new ItemException(
            "Choice entry is wrongly define: action type setEnabled does not contain any values for "
                + cEntryKey
                + ": "
                + actionKey);
      ...
      fa = new FormActionSetValue(actionKey, actionItems[0], list);
    } else if (actionName.equals(SET_ENABLED_ACTION_KEY)) {
      fa = new FormActionSetEnable(actionKey, actionItems[0], actionItems[2].equals("true"));
    } else if (actionName.equals(SELECT_ACTION_KEY)) {
      fa = new FormActionSelectValue(actionKey, actionItems[0], actionItems[2]);
    } else {
      throw new ItemException(
          "Choice entry is wrongly define: action type unknown for "
              + cEntryKey
              + ": "
              + actionKey
              + ": "
  ...
  private ChoiceEntry createChoiceEntry(String iKey, String cEntryKey) {
    ...
    val =
        EZEnvironmentImplem.getString(
            _bundle, ITEM_KEY + iKey + CHOICE_ENTRY_SELECTOR + cEntryKey + NAME_SUFFIX);
    if (val == EZEnvironmentImplem.UNKNOWNSTRING)
      throw new ItemException("Choice entry is wrongly define: name is missing for " + cEntryKey);
    ce.setName(getMessage(val));
    val =
        EZEnvironmentImplem.getString(
            _bundle, ITEM_KEY + iKey + CHOICE_ENTRY_SELECTOR + cEntryKey + KEY_SUFFIX);
    if (val == EZEnvironmentImplem.UNKNOWNSTRING)
      throw new ItemException("Choice entry is wrongly define: key is missing for " + cEntryKey);
    ce.setKey(val);
    val =
        EZEnvironmentImplem.getString(
            _bundle, ITEM_KEY + iKey + CHOICE_ENTRY_SELECTOR + cEntryKey + DESC_SUFFIX);
    if (val == EZEnvironmentImplem.UNKNOWNSTRING)
      throw new ItemException(
          "Choice entry is wrongly define: description is missing for " + cEntryKey);
    ce.setDescription(getMessage(val));
    val =
        EZEnvironmentImplem.getString(
            _bundle, ITEM_KEY + iKey + CHOICE_ENTRY_SELECTOR + cEntryKey + "." + ACTION_KEY);
    if (val != EZEnvironmentImplem.UNKNOWNSTRING) {
  ...
  private Item createItem(String iKey) {
    ...
    try {
      ...
      if (val == EZEnvironmentImplem.UNKNOWNSTRING) throw new ItemException("name is missing");
      item.setName(getMessage(val));
      val = EZEnvironmentImplem.getString(_bundle, ITEM_KEY + iKey + KEY_SUFFIX);
      if (val == EZEnvironmentImplem.UNKNOWNSTRING) throw new ItemException("key is missing");
      item.setKey(val);
      ...
      if (optional == EZEnvironmentImplem.UNKNOWNBOOLEAN) item.setOptional(true);
      else item.setOptional(optional.booleanValue());
      val = EZEnvironmentImplem.getString(_bundle, ITEM_KEY + iKey + CTYPE_SUFFIX);
      if (val == EZEnvironmentImplem.UNKNOWNSTRING) throw new ItemException("CType is missing");
      item.setCtype(Item.getCType(val));
      val = EZEnvironmentImplem.getString(_bundle, ITEM_KEY + iKey + DTYPE_SUFFIX);
      if (val == EZEnvironmentImplem.UNKNOWNSTRING) throw new ItemException("DType is missing");
      item.setDtype(Item.getDType(val));
      if (item.getCtype() == Item.CTYPE_VALUE) setValue(iKey, item);
      else if (item.getCtype() == Item.CTYPE_CHOICE) setChoice(iKey, item);
  ...
  private FormItem createFormItem(String iKey) {
    ...
    try {
      ...
      if (val == EZEnvironmentImplem.UNKNOWNSTRING) item.setKey(iKey);
      else item.setKey(val);
  ...
  /**
   * Returns the set of items (aka Blast parameters). The returned set contains String objects. Use
   * those strings with the method getItem to retrieve the corresponding items (parameters).
   */
  ...
  /** Returns an iterator over FormItem objects. It this iterator is empty, no forms exist. */
  ...
  private String getString(String key) {
    if (_localizedMessages == null) return UNKNOWN;
    try {
