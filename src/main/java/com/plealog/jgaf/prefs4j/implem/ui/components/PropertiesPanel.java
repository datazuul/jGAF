...
import com.plealog.jgaf.prefs4j.implem.ui.model.Choice;
import com.plealog.jgaf.prefs4j.implem.ui.model.ChoiceEntry;
import com.plealog.jgaf.prefs4j.implem.ui.model.DoubleRange;
import com.plealog.jgaf.prefs4j.implem.ui.model.FormAction;
import com.plealog.jgaf.prefs4j.implem.ui.model.FormItem;
import com.plealog.jgaf.prefs4j.implem.ui.model.IntRange;
import com.plealog.jgaf.prefs4j.implem.ui.model.Item;
import com.plealog.jgaf.prefs4j.implem.ui.model.ItemGUIContainer;
import com.plealog.jgaf.prefs4j.implem.ui.model.PropertiesModel;
import com.plealog.jgaf.prefs4j.implem.ui.model.Range;
import com.plealog.jgaf.prefs4j.implem.ui.tools.DesignGridLayout;
import com.plealog.jgaf.prefs4j.implem.ui.tools.IRowCreator;
import com.plealog.jgaf.prefs4j.implem.ui.tools.RowGroup;
...
public class PropertiesPanel extends JPanel {
  ...
  public PropertiesPanel(
      PropertiesModel pm, PreferenceSection conf, HashSet<PreferenceEditor.PROPERTY> props) {
    this();
  ...
  private void initializeGUI() {
    ...
    try {
      ...
      if (_showHelp) headerPanel.add(_headerHelpArea, BorderLayout.SOUTH);
      if (_showHeader || _showHelp) mainPanel.add(headerPanel, BorderLayout.NORTH);
      scroller =
          new JScrollPane(
      ...
      if (_showSaveAsDefBtn) btnPanel.add(saveAsBtn, BorderLayout.WEST);
      if (_showRestoreBtn) btnPanel.add(restoreBtn, BorderLayout.EAST);
   ...
   * @param dtype data type. One of the DTYPE_XXX constants defined in com.korilog.kform.core.Item.
   */
  ...
  /**
   * Helper method to add the content of a FormItem within a container. Note that this container is
      ...
      if (item.getCtype() == Item.CTYPE_VALUE) {
        ...
        if (range != null) {
          ...
          ((JSpinner.DefaultEditor) spinner.getEditor())
              .getTextField()
              .addFocusListener(new GuiCompoFocusListener(item));
        ...
        if (item.getDtype() == Item.DTYPE_FOLDER
          ...
          if (item.getDtype() == Item.DTYPE_FOLDER)
            ...
            btn.addActionListener(
                new ChooseFileAction(guiC.getTxtField(), item.getDtype() == Item.DTYPE_FILES));

          JPanel selectDirectoryPanel = new JPanel();
          new DesignGridLayout(selectDirectoryPanel)
              .margins(0)
              .row()
              .right()
              .add(guiC.getComponent())
              .add(btn)
              .fill();
  ...
  private Component createPropertiesForm() {
    ...
    if (iter.hasNext()) {
      while (iter.hasNext()) {
        ...
        if (iter.hasNext()) formsPanel.add(Box.createRigidArea(new Dimension(0, 8)));
      }
    }
  ...
  private class GuiCompoFocusListener extends FocusAdapter {
    ...
    public void focusGained(FocusEvent e) {
      if (_canUpdateHelp) _helpArea.setText(item.getName() + ": " + item.getDescription());
    }
  }
  ...
  private class ComboSelectionActionListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent event) {
      ...
      if (entry.hasActions() == false) return;
  ...
  private String getValue(String key) {
    ...
    if (guiC.isEnabled() == false) return null;
  ...
  public Properties getValues() throws ItemException {
    if (!_configOk) return null;
    else return (getValues(_pModel.getItemsName()));
  }

  /** Sets defaults values. */
  public void setDefaultValues(Properties values) {
    if (values != null) _defaultProperties = values;
  }
  ...
  private class SaveAsDefaultsAction implements ActionListener {
    public void actionPerformed(ActionEvent event) {
      if (!EZEnvironment.confirmMessage(
          PropertiesPanel.this, EZEnvironment.getMessage("__EZConfDlg.saveDef.msg"))) {
        return;
      }
      try {
  ...
  private class ChooseFolderAction implements ActionListener {
    ...
    public void actionPerformed(ActionEvent event) {
      ...
      if (f != null) _field.setText(f.getAbsolutePath());
    }
  }
