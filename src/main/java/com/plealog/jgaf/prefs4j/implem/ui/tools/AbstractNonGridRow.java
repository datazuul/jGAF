...
abstract class AbstractNonGridRow extends AbstractRow implements INonGridRow {
  ...
  protected int layoutRow(
      LayoutHelper helper, int left, int hgap, int width, int leftFiller, int rightFiller) {
    int x = left;
    int count = _items.size();
    int i = 0;
    int actualHeight = 0;
    for (NonGridRowItem item : _items) {
      ...
      actualHeight =
          Math.max(
              actualHeight,
              helper.setSizeLocation(item.component(), x, compWidth, height(), baseline()));
      x += compWidth + hgap;
      i++;
    }
    return actualHeight;
  ...
  protected abstract int xOffset(int rowWidth, int usedWidth);

  protected abstract int leftFiller(int count, int width, int availableWidth);

  protected abstract int rightFiller(int count, int width, int availableWidth);
