...
import com.plealog.jgaf.prefs4j.implem.core.event.ConfigurationModelListenerSupport;
import java.io.File;
...
public class PreferenceModelImplem extends ConfigurationModelListenerSupport {
  ...
  // 2. si la cle ressource est definie, c'est forcement un fhcier situe dans le meme rep que de
  // descripteur des noeuds
  // 3. l'acces aux elements des noeuds (desc + conf+ defConf) est donne par la cle "locator.root"
  // du descripteur des noeuds
  ...
  public PreferenceModelImplem(String masterConfigLocator, String userConfLocator)
  ...
  private PreferenceSectionImplem readDataNode(
    ...
    if (node.getDataConnector() == null) {
      throw new PreferenceException(
          "undefined data type: " + value + ": for node '" + nodeId + "'");
    }
    node.setConfType(props.getProperty(NODE_PFX + nodeId + NODE_EDT));
    value = getValue(props, NODE_PFX + nodeId + NODE_PARE);
    if (value == null) node.setParent(PreferenceSectionImplem.ROOT_NODE);
    else node.setParent(value);
    ...
    if (value != null) {
      if (userConfLocator == null) node.setConfigurationLocator(confLocator + value);
      else node.setConfigurationLocator(userConfLocator + value);
    }
  ...
  private Map<String, PreferenceSectionImplem> getNodeDataObjects(
      String[] ids, String confLocator, String userConfLocator, Properties props)
      throws PreferenceException {
    ...
    if (dataNodes.isEmpty()) {
      ...
      throw new PreferenceException(
          "unable to find a description for any of nodes: " + str.substring(0, str.length() - 1));
    }
    return dataNodes;
  }
  ...
  private String[] getNodeIds(Properties props) throws PreferenceException {
    ...
    if (ids.length == 0) throw new PreferenceException("configuration does not contain any nodes.");
    return ids;
  }

  private void initialize(String confDir, String userConfLocator, InputStream is)
      throws PreferenceException {
    Properties props;
    ...
    if (value == null) {
      ...
      _resourceLocator =
          confDir + value + "_" + Locale.getDefault().getCountry().toLowerCase() + ".properties";
      if (new File(_resourceLocator).exists() == false) {
        _resourceLocator = confDir + value + ".properties";
        if (new File(_resourceLocator).exists() == false) {
          throw new PreferenceException(
              "unable to read configuration resources: file not found: " + _resourceLocator);
        }
      }
      ...
      try {
        ...
        throw new PreferenceException(
            "unable to read resource locator file: " + _resourceLocator + ": " + e.toString());
      }
    }
  ...
  private void initialize(String masterConfLocator, String userConfLocator)
      throws PreferenceException {
    ...
    try {
      ...
      throw new PreferenceException(
          "unable to read configuration: " + masterConfLocator + ": " + e.toString());
    } finally {
      try {
        if (fis != null) {
          fis.close();
  ...
  public MutableTreeNode getPreferenceSectionsHierarchy() {
    ...
    root =
        new DefaultMutableTreeNode(new PreferenceSectionImplem(PreferenceSectionImplem.ROOT_NODE));
  ...
  private void addParentNode(
      PreferenceSectionImplem curNode, Hashtable<String, DefaultMutableTreeNode> treeNodes) {
