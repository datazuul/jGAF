...
final class GridRow extends AbstractRow implements ISpannableGridRow {
  ...
  @Override
  public ISpannableGridRow indent() {
    return indent(1);
  }

  @Override
  public ISpannableGridRow indent(int n) {
    if (n >= 0) {
      _current.indent(n);
    }
    return this;
  ...
  int gridgap() {
    ...
    for (int i = 0; i < _grids.size() - 1; i++) {
      ...
      if (!(leftGrid.isEmpty() || rightGrid.isEmpty())) {
        ...
        int gap =
            helper.getHorizontalGap(left, right, ComponentPlacement.UNRELATED, parent().parent());
  ...
  int layoutRow(
    ...
    for (SubGrid grid : _grids) {
      ...
      // Layout the sub-grid
      int height = grid.layoutRow(helper, x, height(), baseline(), hgap, width, labelWidth);
