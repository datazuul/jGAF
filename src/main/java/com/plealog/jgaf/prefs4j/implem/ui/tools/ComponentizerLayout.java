...
import com.plealog.jgaf.prefs4j.implem.ui.tools.Componentizer.Builder;
...
final class ComponentizerLayout implements LayoutManager2, Builder {
  ...
  private static void checkNoDuplicateComponents(JComponent[] components) {
  ...
  public void layoutContainer(Container parent) {
    ...
    synchronized (parent.getTreeLock()) {
      ...
      // Prepare layout
      LayoutHelper helper =
          new LayoutHelper(_heightTester, parentWidth, _orientation.isRightToLeft());
  ...
  private static boolean needExtraWidth(ComponentizerItem child, boolean minToPref) {
  ...
  private void computeAll() {
    if (!_inited) {
      ...
      for (int nth = 0; nth < _children.size() - 1; nth++) {
        ...
        int gap =
            helper.getHorizontalGap(left, right, ComponentPlacement.RELATED, _wrapper.parent());
        _gaps[nth] = gap;
        _gap += gap;
      }
      _minWidth += _gap;
