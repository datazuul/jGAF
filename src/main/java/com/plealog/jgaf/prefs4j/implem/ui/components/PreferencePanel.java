...
import com.plealog.jgaf.prefs4j.implem.core.event.ConfigurationFeatureEvent;
import com.plealog.jgaf.prefs4j.ui.PreferenceComponent;
...
public class PreferencePanel extends JPanel implements PreferenceComponent {
  ...
  public PreferencePanel(
      PreferenceModel model, int width, int height, Set<PreferenceEditor.PROPERTY> props)
      throws PreferenceException {
    if (model == null) throw new PreferenceException("preference model is not defined");
    _confModel = model;
  ...
  /** Initializes an editor for every ConfigurationObject contained in the ConfigurationModel. */
    ...
    while (nodes.hasMoreElements()) {
      ...
      if (compo == null) {
        throw new PreferenceException(
            "unable to add ConfigEditor for section: " + node.getName() + ": no GUI.");
      }
  ...
  private void buildGUI(int width, int height) {
    ...
    // not yet available
    _restoreConfig.setVisible(
        false /*_editorProps.contains(ConfigurationEditor.PROPERTY.RESTORE_DEF_BTN)*/);
  ...
  private void notifyListeners(PreferenceEditor editor) {
    ...
    ((PreferenceModelImplem) _confModel)
        .fireConfigurationModelEvent(new ConfigurationFeatureEvent(this, co));
  }
  ...
  public void selectEditor(String editorName) {
    ...
    if (_editors.containsKey(editorName) == false) return;
  ...
  private class CustomIconRenderer extends DefaultTreeCellRenderer {
    ...
    public Component getTreeCellRendererComponent(
      ...
      PreferenceSection nodeObj =
          (PreferenceSection) ((DefaultMutableTreeNode) value).getUserObject();
