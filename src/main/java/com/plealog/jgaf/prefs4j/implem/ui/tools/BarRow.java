...
final class BarRow extends AbstractRow implements IBarRow {
  ...
  @Override
  public IBarRow center(JComponent... children) {
    return add(Tag.OTHER, children);
  }

  @Override
  public IBarRow left(JComponent... children) {
    return add(Tag.LEFT, children);
  }

  @Override
  public IBarRow right(JComponent... children) {
  ...
  int layoutRow(
    ...
    actualHeight = Math.max(actualHeight, layoutOnePart(helper, x, hgap, unrelhgap, _centerItems));

    x = left + rowWidth - computePartWidth(_rightItems, hgap, unrelhgap);
    actualHeight = Math.max(actualHeight, layoutOnePart(helper, x, hgap, unrelhgap, _rightItems));
    ...
    for (BarRowItem item : items) {
      if (item != null) {
        actualHeight =
            Math.max(
                actualHeight,
                helper.setSizeLocation(item.component(), x, _compWidth, height(), baseline()));
        x += _compWidth + hgap;
      } else {
        x += unrelhgap - hgap;
      }
