...
import static com.plealog.jgaf.prefs4j.implem.ui.tools.RowIterator.each;
...
/**
 * The actual {@link LayoutManager} created and installed by {@link DesignGridLayout} on a {@link
 * Container}. This class cannot be instantiated directly, it is made {@code public} so that tools
 * or 3rd-party libraries can detect the use of {@link DesignGridLayout} for the layout of a given
 * {@link Container}.
 *
 * <p>In order to find out the instance of {@link DesignGridLayout} that created a given {@code
 * DesignGridLayoutManager}, you can use {@link #designGridLayout()}.
 *
...
public class DesignGridLayoutManager implements LayoutManager2 {
  ...
  /**
   * Get the instance of {@code DesignGridLayout} that built this {@link LayoutManager}. This
   * enables you to add rows to a given {@link Container} without having the initial {@link
   * DesignGridLayout} that managed layout of that {@code Container}.
   *
   * @return the {@code DesignGridLayout} that created this {@code DesignGridLayoutManager}
   */
  ...
  /**
   * Do not add components via the parent container's {@link Container#add(Component)} method, use
   * directly DesignGridLayout API instead.
   */
  @Override
  ...
  /**
   * Do not remove components via the parent container's {@link Container#remove(Component)} method;
   * removing components from a DesignGridLayout-managed container is not supported.
   */
  @Override
  ...
  public void layoutContainer(Container parent) {
    ...
    synchronized (parent.getTreeLock()) {
      ...
      if (_totalWeight > 0.0) {
        totalExtraHeight = Math.max(0, (parent.getHeight() - _preferredSize.height) / _totalWeight);
      }
      ...
      for (AbstractRow row : _rows) {
        ...
        if (!row.isEmpty()) {
          ...
          row.layout(helper, x, _hgap, _gridgap, _unrelhgap, rowWidth, gridsWidth, _labelWidths);
  ...
  private void computeConsistentBaselineDistance() {
    ...
    while (iter.hasNext()) {
      ...
      if (current.growWeight() == 0.0) {
        int distance = current.height() + current.vgap() - current.baseline() + next.baseline();
        if (current.hasUnrelatedGap()) {
          unrelatedBaselineDistance = Math.max(unrelatedBaselineDistance, distance);
        } else {
          relatedBaselineDistance = Math.max(relatedBaselineDistance, distance);
        }
      }
    }
    ...
    while (iter.hasNext()) {
      ...
      if (current.growWeight() == 0.0) {
        int vgap =
            (current.hasUnrelatedGap() ? unrelatedBaselineDistance : relatedBaselineDistance);
    ...
    for (AbstractRow row : _rows) {
      for (IRowItem item : row.items()) {
        if (item.isFirstSpanRow()) {
          List<AbstractRow> spannedRows = _rows.subList(rowIndex, rowIndex + item.rowSpan());
  ...
  private final DesignGridLayout _designGridLayout;
