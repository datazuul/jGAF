...
final class LayoutHelper {
  ...
  void setHeight(int rowIndex, JComponent component, int spannedRows) {
    ...
    if (MarkerHelper.isMarker(component)) {
      ...
      usedExtraHeight = _tester.computeExtraHeight(component, availableHeight - height);
    }
  ...
  int setSizeLocation(JComponent component, int x, int width, int maxHeight, int maxBaseline) {
    ...
    if (_tester.canGrowHeight(component)) {
      // Checks how much extra height this component can really use
      usedExtraHeight = _tester.computeExtraHeight(component, _availableHeight - height);
    }
