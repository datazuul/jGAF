...
final class SubGrid implements ISubGrid {
  SubGrid(
      List<RowItem> items,
      SubGrid previous,
      ParentWrapper<Container> parent,
      JLabel label,
      int gridspan) {
    _items = items;
    _previous = previous;
  ...
  private static final String NO_PREVIOUS_SUBGRID =
  ...
  private static final String NO_MATCHING_COMPONENT =
      "spanRow() cannot work when there is no component, on the above grid-row, "
          + "with a matching column location";
  private static final String UNMATCHED_COLUMNS_SUBGRIDS =
  ...
  private final ParentWrapper<Container> _parent;
