...
import com.plealog.jgaf.wizard.ui.WizardListener;
import com.plealog.jgaf.wizard.ui.WizardListenerAdapter;
...
/**
 * This class describes the data model of a WizardDialog. It is actually made of one or more
 * WizardStepModel(s).
 */
public abstract class WizardModel {
  ...
  public WizardModel(WizardStepModel firstStep) {
    ...
    this.stepListener =
        new WizardListenerAdapter() {

          @Override
          public void update() {
            super.update();
            WizardModel.this.fireFinishable(
                WizardModel.this.isFinishable(WizardModel.this.stepHistory));
    ...
    this.stackListener =
        new StackListener<WizardStepModel>() {
          @Override
    ...
    if ((oldStep != newStep)) {
      ...
      if (oldStep != null) {
        ...
        if (!oldStepAlreadyCommitted) {
          if (!oldStep.getView().commit()) return;
        }
