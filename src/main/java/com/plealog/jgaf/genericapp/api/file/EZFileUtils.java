...
public class EZFileUtils {
  ...
  /**
   * Check whether or not a path terminates with a separator. If not, this method returns a path
   * ending with such separator.
   */
  public static String terminatePath(String path) {
    if (path == null || path.isEmpty()) return path;
    if (path.charAt(path.length() - 1) == File.separatorChar) return path;
  ...
  /** Returns the content of a file as text. This method should be used only with text files. */
  ...
  /**
   * Returns the file name (without extension) given a absolute path. If path denotes a directory,
   * then the last element of the absolute path is returned. It path denotes a file, then the last
   * element of the absolute path is returned but without its extension (if any). The file extension
   * is recognized by looking for the last occurrence of a dot. Note that the method may return null
   * if no file name can be found.
   */
   ...
   * @param ext the file extension (do not provide the prefix dot).
   */
  public static File forceFileExtension(File f, String ext) {
    String path;

    path = f.getAbsolutePath();
    if (path.endsWith(ext) == false) path += "." + ext;
    return new File(path);
  }

  /**
   * Uncompress a gzipped file. A file is considered gzipped if its extension matches '.gz'. If it
   * is not true, then the zipFile param is returned as is.
   *
