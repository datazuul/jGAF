...
import com.plealog.jgaf.genericapp.api.configuration.DirectoryManager;
import com.plealog.jgaf.genericapp.api.configuration.DirectoryManagerException;
import com.plealog.jgaf.genericapp.api.configuration.DirectoryType;
      ...
      if (_code == 3) {
        ...
        try {
          ...
          EZFileUtils.copyFile(
              new File("build.xml"), new File(path + "build.xml") // "path" ends with File.separator
              );
        } catch (IOException ex) {
          throw new DirectoryManagerException(
              "Unable to prepare web template directory: " + ex.toString());
        }
      }
    }
