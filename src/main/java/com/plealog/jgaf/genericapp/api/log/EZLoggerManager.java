...
/**
 * Utility class to setup logging system. This class is a convenient wrapper to setup default Java
 * Logging system.
 */
public class EZLoggerManager {
  ...
  /**
   * Set the log level. This method can be called before and after initialize() method. Default is
   * INFO level. EZLoggerManager only handles SEVERE, WARNING, INFO and CONFIG levels. CONFIG level
   * is used as a DEBUG-like level.
   *
  ...
  /**
   * Figures out whether or not the default Java Logging Console logger has to be enabled. Default
   * is true.
   *
  ...
  /**
   * Figures out whether or not the UI logger has to be enabled. Default is false.
   *
  ...
  /**
   * Figures out whether or not a file logger has to be enabled. Default is false.
   *
   * @param fileName the file name to use. Parameter can be a simple file name, or a full path to a
   *     file.
   ...
   *
   * <p>See java.util.logging.SimpleFormatter.
   */
  ...
  /**
   * Call this method to setup the logging framework. A call to this method has to be done after
   * calling the various enableXXX() and setXXX() methods of this class, as needed.
   */
  ...
  public static JComponent getUILogger() {
    if (txtAreaHandler != null) return txtAreaHandler.getComponent();
    else return null;
  }
}
