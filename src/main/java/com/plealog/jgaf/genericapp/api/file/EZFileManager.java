...
import com.plealog.jgaf.genericapp.implem.file.EZFileExtSelector;
...
public class EZFileManager {
  ...
  private static boolean _useOSNativeFileDialog =
   ...
   *
   * @param useNativeOSFileDialog passes in true to use {@link java.awt.FileDialog}, false to use
   *     {@link javax.swing.JFileChooser}. The former enables the display of an OS-dependent File
   *     Chooser, while the latter is the Java Swing specific File CHooser.
   */
  ...
  public static String getDefaultPath() {
    ...
    if (szCurPath == null) szCurPath = ".";
    return (szCurPath);
  }

  /**
   * Changes the default directory used in File Chooser dialogue boxes. At startup, this default
   * path is set to the user home directory.
   */
  public static void setDefaultPath(String path) {
    if (path != null) _szDefaultPath = path;
  }

  /**
   * Gets the user home directory. This method takes into account workaround defined by
   ...
   */
  public static File chooseFileForOpenAction(
      Component parent, String dialogTitle, EZFileFilter ff) {
    File[] files;
  ...
  private static File[] chooseFilesForOpenAction(
      Component parent, String dialogTitle, EZFileFilter ff, boolean enableMultipleSelection) {
    ...
    if (_useOSNativeFileDialog) {
      ...
      // dlg title
      if (dialogTitle != null && dialogTitle.length() > 0) jfc.setTitle(dialogTitle);
      else jfc.setTitle("Open");
      // default directory
   ...
   * @param dialogTitle title of the dialogue box
   * @param ff a file filter. Use {@link EZFileTypes} to manage file filters.
   * @param types list of file types to restrict display of the dialog box to them
   * @return a file or null if dialogue has been canceled.
   */
  public static File chooseFileForSaveAction(
      Component parent, String dialogTitle, EZFileFilter ff, List<EZFileExtDescriptor> types) {
    File file = null;
    ...
    if (_useOSNativeFileDialog) {
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
        ...
        if (extSelector != null) {
          file =
              EZFileUtils.forceFileExtension(file, extSelector.getSelectedFType().getExtension());
        }
      }
    }
    return file;
  ...
  public static File chooseDirectory(Component parent, String dialogTitle, EZFileFilter ff) {
    ...
    if (_useOSNativeFileDialog) {
      ...
      // dlg title
      if (dialogTitle != null && dialogTitle.length() > 0) jfc.setTitle(dialogTitle);
      else jfc.setTitle("Select a directory");
      ...
      // dlg title
      if (dialogTitle != null && dialogTitle.length() > 0) jfc.setDialogTitle(dialogTitle);
      else jfc.setDialogTitle("Select a directory");
      // default directory
      jfc.setCurrentDirectory(new File(EZFileManager.getDefaultPath()));
      jfc.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
      jfc.setFileHidingEnabled(false);
      if (jfc.showOpenDialog(parent == null ? EZEnvironment.getParentFrame() : parent)
