...
public class ImageCapture {
  ...
  public static BufferedImage createImage(JComponent component, String fileName)
      throws IOException {
    if (component instanceof JTable) return createTableImage((JTable) component, fileName);
   ...
   * @exception IOException if an error occurs during writing
   */
  public static BufferedImage createImage(JComponent[] components, String fileName)
      throws IOException {
    ...
    if (components[1] != null) dColHeader = components[1].getSize();
    else dColHeader = new Dimension(0, 0);
    ...
    if (components[1] != null) components[1].paintAll(g2d);
    g2d.translate(0, dColHeader.height + decal);
  ...
  private static void checkMemory(int w, int h) throws OutOfMemoryError {
    ...
    long imSize = w * h;
    if (imSize > freeMem) throw new OutOfMemoryError("Not enough memory to create an image.");
  }

  private static BufferedImage createTableImage(JTable table, String fileName) throws IOException {
    ...
    image =
        new BufferedImage(
            dTable.width, dHeader.height + dTable.height + 1, BufferedImage.TYPE_INT_RGB);
    g2d = image.createGraphics();
    if (tabHeader != null) {
   ...
   * @exception IOException if an error occurs during writing
   */
  public static BufferedImage createImageFromComponent(Component component, String fileName)
      throws IOException {
    if (component == null) {
      return null;
    }
