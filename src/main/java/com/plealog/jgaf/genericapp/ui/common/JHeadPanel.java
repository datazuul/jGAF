...
public class JHeadPanel extends JPanel {
  ...
  /**
   * Constructs a <code>JHeadPanel</code> with the specified icon, and title.
   *
  ...
  /**
   * Constructs a <code>JHeadPanel</code> with the specified title, tool bar, and content panel.
   *
   * @param title the initial title
  ...
  /**
   * Constructs a <code>JHeadPanel</code> with the specified icon, title, tool bar, and content
   * panel.
   *
   * @param icon the initial icon
   * @param title the initial title
   ...
   * @param content the initial content pane
   */
  public JHeadPanel(Icon icon, String title, Component content, boolean folder, boolean folded) {
    ...
    if (_folded) cardlayout.show(_cardPane, FOLDEDPANE);
    else cardlayout.show(_cardPane, EXTENDEDPANE);
    ...
    if (folder) foldPanel.add(_foldButton, BorderLayout.WEST);
  ...
  public void setTitleFont(Font fnt) {
    if (fnt != null) this._titleLabel.setFont(fnt);
  }
  ...
  public void setContent(Component newContent) {
    ...
    if (_content != null) _pane.remove(_content);
    oldContent = _content;
    _content = newContent;
    if (newContent != null) _pane.add(newContent, BorderLayout.CENTER);
  ...
  private JPanel buildHeader(JLabel label) {
    _gradientPanel = new GradientPanel(getHeaderBackground(), UIManager.getColor("control"));
  ...
  private Dimension getHeaderSize() {
    ...
    dim =
        new Dimension(
            dim.width + insets.left + insets.right, dim.height + insets.top + insets.bottom);
    return (dim);
  }
      ...
      addMouseListener(
          new MouseAdapter() {
            public void mouseEntered(MouseEvent e) {
              ...
              else backColor = BK_COLOR_REF.darker();
              repaint();
            }
  ...
  private class FoldAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent actionevent) {
      ...
      if (_folded) cardlayout.show(_cardPane, EXTENDEDPANE);
      else cardlayout.show(_cardPane, FOLDEDPANE);
