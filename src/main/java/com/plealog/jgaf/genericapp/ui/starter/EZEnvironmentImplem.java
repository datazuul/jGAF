...
public abstract class EZEnvironmentImplem {
  ...
  /** generic font for the entire GUI */
  private static final Font MAINFONT = new Font("SansSerif", Font.PLAIN, 12);
  ...
  private static final String SYSTEM_BUNDLE_NAME =
      Accessor.class.getPackage().getName() + ".messages";
  ...
  private static final ResourceBundle RESOURCE_BUNDLE =
      ResourceBundle.getBundle(SYSTEM_BUNDLE_NAME);
   ...
   * @return the value associated to key. Returns "?" is key if not found or not defined, or if
  ...
  public static void setSystemTextColor(Color color) {
    if (color != null) _systemTextColor = color;
  }
   ...
   * @param bundle a resource bundle
   * @param key a resource key
   * @return the value associated to key. Returns 0 (zero) if key is not found or not defined, or if
   *     Properties file is not found.
   */
   ...
   * @param bundle a resource bundle
   * @param key a resource key
   * @return the value associated to key. Returns false if key is not found or not defined, or if
   *     Properties file is not found.
   */
  public static Boolean getBoolean(ResourceBundle rb, String key) {
    ...
    try {
      szVal = rb.getString(key);
      szVal = szVal.trim();
      if (szVal.toLowerCase().equals("true")) val = Boolean.TRUE;
      else if (szVal.toLowerCase().equals("false")) val = Boolean.FALSE;
    } catch (MissingResourceException mre) {
      // do nothing here
    } catch (NullPointerException npe) {
      EZLogger.warn("Key is not defined: " + npe.getMessage());
  ...
  public static Double getDouble(ResourceBundle rb, String key) {
    ...
    try {
      ...
      EZLogger.warn("Value for key '" + key + "' does not contain a double: " + nfe.getMessage());
    }
    return (val);
  }
  ...
  private static Color convertColor(String szColor) {
    ...
    if (tokenizer.hasMoreTokens() == false) return Color.black;
   ...
   * @param bundle a resource bundle
   * @param key a resource key
   * @return the value associated to key. Returns Color.black if key is not found or not defined, or
  ...
  public static Font getMainFont(ResourceBundle rb) {
    ...
    try {
      ...
      EZLogger.warn("Value for font size does not contain an integer: " + nfe.getMessage());
    }
    return (fnt);
  }
  ...
  /**
   * Returns an Image given its name, null if not found. See this.getImageIcon() for more
   * information.
   */
   ...
   *
   * @param imageName an image name which just consists of name follows by a period and an
   *     extension. Example: new.gif. If an absolute path is required to get the image, consider
   *     adding the path using this.addSearchPath().
   */
  public static ImageIcon getImageIcon(String imageName) {
    ...
    if (imageName != null && imageName.length() > 1) {
      ...
      // try by path
      for (i = 0; i < size; i++) {
        url = EZEnvironmentImplem.class.getResource(_imgPaths.get(i) + imageName.trim());
        if (url != null) {
  ...
  public static ImageIcon loadImageIcon(File f) {
    ...
    if (f == null || f.exists() == false || f.length() == 0l) return null;
    try {
  ...
  /**
   * Adds a path in the Image search path list. A 'path' is actually the name of a package that is
   * supposed to also contain image ressources. A path has to start with '/' and has to use '/'
   * instead of '.'. Example: you can pass in here "/my/package/" when you want to get images from
  ...
  /**
   * Adds a Class in the Image search Class list. This is to be used when application is shipped
   * within a JAR: search path seems not to work, so use this method to add a class that is located
   * is the same directory as the image to load.
   */
  ...
  public static String[] tokenize(String input, String delim) {
    ...
    if (tokenizer.countTokens() == 0) return new String[] {};
  ...
  public static EZActionManager getActionsManager() {
    if (_actionManager == null) {
      try {
        _actionManager =
            new EZActionManager(
                _userDefinedActionsBundle != null
                    ? _userDefinedActionsBundle
                    : ResourceBundle.getBundle(Accessor.class.getPackage().getName() + ".ui"));
      } catch (Exception ex2) {
        _actionManager = null;
  ...
  public static void setHandCursor() {
    if (_parent != null) _parent.setCursor(HAND_CURSOR);
  }

  public static void setDefaultCursor() {
    if (_parent != null) _parent.setCursor(NORMAL_CURSOR);
  }

  public static void setWaitCursor() {
    if (_parent != null) _parent.setCursor(WAIT_CURSOR);
  }
  ...
  public static boolean saveProperties(Properties props, OutputStream os, String comments) {
    boolean bRet = false;
    ...
    try {
      ...
      EZLogger.warn(EZEnvironmentImplem.getMessage("__EZKResources.err4") + ": " + ex);
    }
    return bRet;
  }
