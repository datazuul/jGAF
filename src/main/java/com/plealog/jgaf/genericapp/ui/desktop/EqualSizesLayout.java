...
/**
 * A layout manager which places equally sized items in a single row that has a maximum size. Useful
 * for task bar like layouts. Original author was Santhosh Kumar at
 * https://myswing.dev.java.net/MyBlog/
 */
public class EqualSizesLayout implements LayoutManager, SwingConstants {
  ...
  /**
   * Get the preferred size of the biggest component and the size of the overall layout.
   *
  ...
  private Dimension[] dimensions(Component children[]) {
    ...
    return new Dimension[] {
      new Dimension(maxWidth, maxHeight), new Dimension(usedWidth, usedHeight),
    };
  }
  ...
  public void layoutContainer(Container container) {
    ...
    if (usedWidth > allowedWidth) {
      ...
      maxWidth = (allowedWidth / children.length) - (gap * children.length);
    }
      ...
      case TOP:
        for (int i = 0, c = children.length; i < c; i++) {
          ...
          children[i].setBounds(
              insets.left + (maxWidth + gap) * i, insets.top, maxWidth, maxHeight);
        }
        break;
      case RIGHT:
      case BOTTOM:
        for (int i = 0, c = children.length; i < c; i++) {
          if (!children[i].isVisible()) continue;
          children[i].setBounds(
              container.getWidth() - insets.right - usedWidth + (maxWidth + gap) * i,
              insets.top,
              maxWidth,
              maxHeight);
        }
  ...
  public Dimension preferredLayoutSize(Container container) {
    ...
    return new Dimension(
        insets.left + usedWidth + insets.right, insets.top + usedHeight + insets.bottom);
  }
