package com.plealog.jgaf.genericapp.ui.menu;

import com.plealog.jgaf.genericapp.api.EZApplicationBranding;
import com.plealog.jgaf.genericapp.api.EZDefaultActionHandler;
...
/**
 * EZActionManager handles the creation of menu and tool bars given a resource bundle file. This
 * file is passed to the constructor of this class in order to automatically creates a set of
 * Actions (read from the bundle file) which are then used to build a menu and a tool bars.
 */
public class EZActionManager implements PropertyChangeListener {
   ...
   * Default constructor.
   *
   * @param bundle the resource bundle 'file' containing the definition of the menu/tool bars.
   * @throws Exception if the resource bundle cannot be located or if this bundle contains wrongly
   *     formatted information.
   */
  public EZActionManager(ResourceBundle bundle) throws Exception {
    this();
    if (bundle == null) throw new NullPointerException("Resource bundle is not defined.");
    _bundle = bundle;
    initialize();
  }
  ...
  /**
   * Figures out if a particular Action can be displayed in Menus for the current OS. That OS is
   * actually identified by the String osTag.
   */
  private boolean validForThisOS(String actionName, String osTag) {
    ...
    if (osList.equals("?")) return true;
    if (osList.indexOf("all") >= 0) return true;
    if (osList.indexOf(osTag) >= 0) return true;
    return false;
  }
  ...
  /**
   * Helper method to set a text/icon/tooltip to a button. This method gets all resources from the
   * resource bundle.
   */
  private boolean createToolbarBtn(
      ResourceBundle resource, AbstractButton button, String actionCommand) {
    Icon icon;
    String tooltip;

    if (resource == null || button == null || actionCommand == null) return false;

    icon =
        EZEnvironmentImplem.getImageIcon(
            EZEnvironmentImplem.getString(resource, actionCommand + SUFFIX_ICON));
    if (icon != null) {
      button.setIcon(icon);
      button.setText(null);
    } else {
      button.setIcon(null);
      button.setText(EZEnvironmentImplem.getString(resource, actionCommand + SUFFIX_LABEL));
    }
    button.setFont(EZEnvironmentImplem.getMainFont(null));
    tooltip = EZEnvironmentImplem.getString(resource, actionCommand + SUFFIX_TIP);
  ...
  /**
   * Helper method to set a text/icon/tooltip to a menu item. This method gets all resources from
   * the resource bundle.
   */
  private boolean initMenuItem(ResourceBundle resource, JMenuItem item, String actionCommand) {
    String tooltip, accel, mnemonic;
    KeyStroke stroke = null;
    Icon icon;
    int val;

    if (resource == null || item == null || actionCommand == null) return false;

    item.setText(EZEnvironmentImplem.getString(resource, actionCommand + SUFFIX_LABEL));
    ...
    accel = EZEnvironmentImplem.getString(resource, actionCommand + SUFFIX_ACCELERATOR);
    if (accel.equals("?") == false && accel.length() > 0) {

      int idx = accel.indexOf(" ");
      if (idx == -1) { // accel is a single char
        // from:
        // http://alvinalexander.com/apple/mac/java-mac-native-look/Converting_Control_keystrok.shtml
        val = getKey(accel.charAt(0));
        if (val != -1) {
          stroke =
              KeyStroke.getKeyStroke(val, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask());
        } else {
    ...
    mnemonic = EZEnvironmentImplem.getString(resource, actionCommand + SUFFIX_MNEMONIC);
  ...
  private Component createMenuItem(ResourceBundle resource, String key, ActionMap actions) {
    String subMenu;
    String[] itemKeys;
    EZBasicAction a;
    JMenuItem item;

    subMenu = EZEnvironmentImplem.getString(resource, key + SUFFIX_MENU);
    ...
    if (subMenu.equals("?") == false) {
      ...
      if (a == null || !a.isValidForThisOsMenu()) return null;
  ...
  private JMenu createMenu(
      ResourceBundle resource, String key, String[] itemKeys, ActionMap actions) {
    Component compo;
    ...
    mName = EZEnvironmentImplem.getString(resource, key + SUFFIX_TEXT);
    menu.setText(mName);
    ...
    mnemonic = EZEnvironmentImplem.getString(resource, key + SUFFIX_MNEMONIC);
    if (!mnemonic.equals("?") && mnemonic.length() > 0) menu.setMnemonic(mnemonic.toCharArray()[0]);
  ...
  public JMenuBar createMenubar() {
    ...
    for (i = 0; i < menuKeys.length; i++) {
      itemKeys = tokenize(EZEnvironmentImplem.getString(_bundle, menuKeys[i] + SUFFIX_MENU));
   ...
   *
   * <p>By default, this method only handles message for the property
   * DDBasicAction.GLOBAL_UPDT_PROP. When receiving such a message, this method just call
   * this.update(), i.e. it asks all Actions to update themselves.
   */
    ...
    public void handlePreferences() {
      ...
      if (confFile != null) {
        try {
          ...
          PreferenceUIFactory.showPreferenceDialog(
              null,
              EZApplicationBranding.getAppName()
                  + " "
                  + EZEnvironmentImplem.getMessage("__EZ.msg3"),
              model);
        } catch (Exception e) {
        ...
        EZEnvironment.displayInfoMessage(
            EZEnvironment.getParentFrame(), EZEnvironmentImplem.getMessage("__EZ.err2"));
      }
    }
  }
    ...
    try {
      if (Character.isLetter(ch)) {
        val =
            Integer.valueOf(
                KeyEvent.class
                    .getDeclaredField("VK_" + Character.toUpperCase(ch))
                    .get(String.class)
                    .toString());
      } else if (Character.isDigit(ch)) {
        val =
            Integer.valueOf(
                KeyEvent.class.getDeclaredField("VK_" + ch).get(String.class).toString());
      }
