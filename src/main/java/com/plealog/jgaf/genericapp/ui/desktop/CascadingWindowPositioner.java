...
/**
 * Positions the windows in a JDesktopPane MDI by cascading them down one by one from the upper
 * left. This class only sets the positions, the size is set by the window itself.
 */
public class CascadingWindowPositioner implements WindowPositioner {
  ...
  public Point getPosition(JInternalFrame newFrame, List<JInternalFrame> frames) {
    int x = 0;
    int y = 0;
