...
/**
 * A complete implementation of a Windows-style taskbar for the JDesktopPane multiple document
 * interface (MDI) under Swing. Based on a blog entry by Patrick Gotthardt at
 * http://www.jroller.com/page/pago/.
 */
public class JTaskBar extends JPanel {
   ...
   *
   * @param desktop The desktop to create a task bar for.
   */
   ...
   *
   * @param frm The frame to check for.
   ...
   *
   * @param act The action for the button being created.
   */
  ...
  /**
   * The action which will be invoked when a given button is selected by operating on the given
   * frame.
   */
  private class TaskPaneAction extends AbstractAction implements InternalFrameListener {
    ...
    /**
     * Create an action that dictates the behavior of the button for a given frame in the desktop.
     *
     * @param frm The frame for the task bar button
     */
    ...
    /** @param btn The new button for this frame. */
    ...
    /**
     * Button was pushed so either select the window if not selected, or iconify it if selected.
     *
  ...
  /**
   * The contextual menu on the task bar buttons allowing for the user to minimize, maximize,
  ...
  private class WindowPopupMenu extends JPopupMenu {
     ...
     *
     * @param frame The frame the task bar button controls.
     */
    WindowPopupMenu(final JInternalFrame frame) {
      this.frame = frame;
      restoreMenuItem = new JMenuItem("Restore");
      restoreMenuItem.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              try {
                if (frame.isMaximum()) {
    ...
    /**
     * Toggle the enabled state of the restore, minimize, and maximize buttons depending on the
     * current state of the monitored frame.
     */
  ...
  /**
   * Custom button which places it's tooltip above itself. Otherwise the default tooltip position
   * means it is covered by the Windows task bar when the application is maximized.
   */
  private class TaskButton extends JToggleButton {
    ...
    /**
     * Regular tooltip placement hides the tooltip under the Windows taskbar when the main window is
     * maximized. This will ensure it is visible.
     *
